...
bool isMagicSquare(int** matrix, int N)
{
    ...
    // compute row sums
    #pragma omp parallel target distribute for num_threads(loops) shared(row_sums) shared
    for (int i = 0; i < N; i++)
    {
...
/*     if (!allEqual(row_sums, N)) {
    ...
    // compute column sums
    #pragma omp parallel target distribute for num_threads(loops) shared(col_sums) shared
    for (int i = 0; i < N; i++)
    {
